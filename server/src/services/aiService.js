...
class AIService {
  ...
  buildPrompt({ content, moodRating, userPreferences, recentEntries, isPremium }) {
    const focusAreasText = userPreferences.focusAreas.join(', ');
    const moodText = this.getMoodDescription(moodRating);
    ...
    const premiumContext = isPremium
      ? '\n\nThis is a PREMIUM user - provide deeper, more detailed insights with pattern analysis.'
      : '\n\nThis is a FREE user - provide concise but meaningful insights.';

    return `You are an empathetic AI therapist analyzing a personal journal entry. Provide ONE specific, actionable insight.

User Context:
- Focus areas: ${focusAreasText}
- Current mood: ${moodText}
- Personality type: ${userPreferences.personalityType}${premiumContext}
...
{
  "insight": "Your ${isPremium ? '75-150' : '50-100'} word supportive insight with specific observations",
  "followUpQuestion": "One thoughtful follow-up question to encourage deeper reflection",
  "confidence": 0.85
}

Requirements:
- Be supportive, empathetic, and non-clinical
- Include specific observations about their entry
- Confidence should be between 0.7-0.95
- Keep tone conversational and encouraging
- ${isPremium ? 'Provide deeper pattern analysis and connections' : 'Focus on immediate, actionable insights'}`;
  }
